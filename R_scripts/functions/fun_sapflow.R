...
treeScaleSimple1 <- function(data, depths, swd) {
   ...
   Aring = do.call(rbind,
                   lapply(2:length(depths), function(x)
                      data.frame(x = depths[x - 1], 
                                 Aring = pi * abs(depths[x] ^ 2 - (depths[x - 1]) ^ 2))))
   Aring = rbind(Aring, data.frame(
      x = tail(depths, 1),
      Aring = pi * abs(tail(depths, 1) ^ 2 - swd ^
                          2)
   ))
   print(Aring)
...
treeScaleSimple3 <- function(data, depths, swd) {
   ...
   Cring = do.call(rbind,
                   lapply(2:length(depths), function(x)
                      data.frame(x = depths[x - 1], Cring = pi * abs(depths[x] + (depths[x -
                                                                                            1])))))
